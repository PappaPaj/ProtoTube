...
const hideShorts = document.getElementById("hideshorts-toggle");
...
chrome.storage.sync.get("hideshorts-toggle", (val) => {
  if (val["hideshorts-toggle"] != undefined) {
    hideShorts.checked = val["hideshorts-toggle"];
  }
});
    ...
    (tabs) => {
      ...
      chrome.tabs
        .sendMessage(tabs[0].id, {
          from: "popup",
          subject: "settings",
          subOnly: subOnly.checked,
          noShorts: noShorts.checked,
...
hideShorts.addEventListener("input", function (e) {
  var setting = e.target.checked;
  chrome.storage.sync.set({ "hideshorts-toggle": setting }, function () {
    console.log("Value is set to " + setting);
  });
  sendUpdatedSettings();
});
